...
import { createContact } from '../db/contacts.db.js';
import { createApplication } from '../db/careers.db.js';
import { addSubscriber } from '../db/newsletter.db.js';
...
export async function handle(kind, method, body, env, ip) {
  ...
  if (result.status === 200 && result.body.data) {
    try {
      if (kind === 'contact') {
        await createContact(result.body.data, env);
      } else if (kind === 'apply') {
        await createApplication(result.body.data, env);
      } else if (kind === 'newsletter') {
        await addSubscriber(result.body.data.email, env);
      }
