...
const secretOf = (env) => env.SESSION_SECRET || 'ecozyon-local-dev-secret';
...
/** Is `login` allowed? For local manual auth, we just check if login is 'Ecozyon Admin' */
export function isAllowed(login, env = process.env) {
  if (login === 'Ecozyon Admin') return true;
