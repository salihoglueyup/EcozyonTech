      ...
      <div className="mx-auto max-w-7xl px-6">
            ...
            return (
              ...
              >
                ...
                <div className="text-left">
                  <div className={`text-[13.5px] font-semibold ${isActive ? "" : "text-slate-700 dark:text-slate-300"}`}>{per.label}</div>
                  <div className="text-[11px] text-slate-500 dark:text-slate-400 leading-tight max-w-[200px] truncate">{per.tagline}</div>
                </div>
        ...
        <Reveal delay={100} className="eco-card relative overflow-hidden rounded-3xl shadow-[0_24px_70px_-40px_rgba(15,23,42,.3)]">
          ...
          <div className="relative grid grid-cols-1 lg:grid-cols-[1.1fr_1fr]">
            {/* Left: copy + features */}
            <div className="p-7 lg:p-10 border-b lg:border-b-0 lg:border-r border-slate-900/[.05] dark:border-white/[.06]">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10.5px] uppercase tracking-[.14em] text-slate-500 dark:text-slate-400 font-semibold">
                  {lang === "tr" ? "Kim için" : "Built for"}
                </span>
              </div>

              <h3 className="mt-4 font-display text-[clamp(1.6rem,3vw,2.4rem)] leading-[1.1] tracking-tight text-slate-900 dark:text-slate-100">
                {p.tagline}
              </h3>
              <p className="mt-3 text-[14px] text-slate-600 dark:text-slate-400 leading-relaxed">{p.who}</p>
          ...
          <div className="relative grid grid-cols-2 lg:grid-cols-4 divide-y lg:divide-y-0 lg:divide-x divide-slate-900/[.05] border-t border-slate-900/[.05] dark:border-white/[.06]">
            {p.metrics.map((m, i) => (
              <div key={i} className="px-6 py-4">
                <div className="text-[10.5px] uppercase tracking-[.14em] font-semibold text-slate-500 dark:text-slate-400">{m.label}</div>
                <div className="mt-1 font-display text-[24px] tracking-tight" style={{ color: p.color }}>{m.val}</div>
              </div>
            ))}
            <div className="px-6 py-4">
              <div className="text-[10.5px] uppercase tracking-[.14em] font-semibold text-slate-500 dark:text-slate-400">{lang === "tr" ? "Fiyat" : "Pricing"}</div>
              <div className="mt-1 font-mono text-[14px] text-slate-900 dark:text-slate-100">{p.price}</div>
            </div>
            <div className="px-6 py-4 hidden lg:block">
              <div className="text-[10.5px] uppercase tracking-[.14em] font-semibold text-slate-500 dark:text-slate-400">{lang === "tr" ? "Lansman" : "Launch"}</div>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-3">
        <div className="text-[10.5px] uppercase tracking-[.14em] font-semibold text-slate-500 dark:text-slate-400">
      ...
      <div className="mt-3 rounded-2xl border border-slate-900/[.06] p-3 flex items-center justify-between" style={{ backgroundColor: `${color}10` }}>
        <div>
          ...
          <div className="text-[13px] font-medium text-slate-800 dark:text-slate-200 mt-0.5">{lang === "tr" ? "Bisiklet haftası" : "Bike week"}</div>
        </div>
        <div className="text-right">
          <div className="text-[11px] font-mono text-slate-600 dark:text-slate-400">3d 12h {lang === "tr" ? "kaldı" : "left"}</div>
          <div className="text-[10px] text-slate-500 dark:text-slate-400">{lang === "tr" ? "12/15 katılımcı" : "12/15 participants"}</div>
        </div>
      </div>
    </div>
  );
